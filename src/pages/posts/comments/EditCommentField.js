...
export default class EditCommentField extends Component{
  ...
  handleInnitializeDropzone=dropzone=>{
    const {comment:{image}} =this.props
        if(image){
...
myDropzone.addFile(file);
  ...
  render(){
   ...
   const {comment,open4Edit,handleDiscardChanges,onChange,handleSaveChanges,handleAddedFile,handleRemovedFile}=this.props
   const eventHandlers = { 
    ...
    <Confirm
          ...
          content='This comment appears to be blank. Please write something or attach a photo to post.'
          open={open4Empty}
